...
function ClearanceOrgs(props) {
    ...
    const addOrgs = (e) => {
        ...
        let OrgsDetails = {
            organizetionName: values.txt_name,organizetionCode: values.txt_code,address: values.txt_addresh,
             phoneNumber: values.txt_Phon_no, mailId: values.txt_emailid, website: values.txt_website, tinNumber: values.txt_tin_no,panNumber:values.txt_Pan_no, yearOfEstablishment: values.txt_year_of_Established
        };
        ...
        PlaceServices.addClearanceOrganizetion(OrgsDetails).then((res) => {
            setValues({
    ...
    const updateOrgs = (e) => {
        ...
        if(!values.txt_name || !values.txt_code || !values.txt_year_of_Established || values.tbl_point_Id === 0)
        {
            setValues({
                txt_Orgsid: 0,
                txt_addresh: "",
        ...
        console.log("point " + values.tbl_point_Id)
        console.log("OrgsDetails " + JSON.stringify(OrgsDetails))
        PlaceServices.updateClearanceOrganizetion(values.tbl_point_Id,OrgsDetails).then((res) => {
            setValues({
                txt_infoMassage: res.data.message,
                txt_Orgsid: 0,
                txt_addresh: "",
    ...
    const deleteOrgs = (e) => {
        ...
        PlaceServices.deleteClearanceOrganizetion(values.tbl_point_Id).then((res) => {
            setValues({
                txt_infoMassage: res.data.message,
                txt_Orgsid: 0,
                txt_addresh: "",
    ...
    const editOrgs = (Orgs) => {
        console.log("logo "+JSON.stringify(Orgs.organizationLogo))
        setValues({
            ...
            <Form noValidate validated={validated} style={{ fontFamily: "serif", overflowX: "hidden" }}>
                <Row>
                    <Col xs={11} style={{ margin: "auto" }} className=" font-serif">
                        <Form.Text style={{ fontSize: "20px", color: "black" }}>Clearance Organizetion</Form.Text>
                        <Card style={{ borderTop: "4px solid rgb(175, 174, 129)", borderLeft: "3px solid rgb(128, 127, 96)" }}>
                            ...
                            <Form.Group as={Col} md="4" controlId="validationFormik101" className="position-relative">
                                ...
                                {/*  */}
                                <Row xs={5} className="mx-4" style={{marginTop:"-60px"}}>
                                <Form.Group as={Col} md="4" controlId="validationFormik101" className="position-relative">
                        ...
                        <ButtonToolbar className="mx-5" bsPrefix >
                            <Button onClick={addOrgs} disabled={submit() + loading} className="btn btn-info p-2" type="submit">
                            {loading ? 'processing...' : 'submit'}
                            </Button>
                            <Button onClick={updateOrgs} className="btn btn-info p-2" disabled={isDisabled() + LodingUd}>{LodingUd ? 'Processing...' : 'update'}</Button>
                            <Button className="btn btn-info p-1" onClick={cancle}>Reset</Button>
                            <Button  onClick={deleteOrgs} className="btn btn-danger p-2" disabled={isDisabled() + LodingDelete}>{LodingDelete ? 'Processing...' : 'delete'}</Button>
                        </ButtonToolbar >
